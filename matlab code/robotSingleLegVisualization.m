...
for k = 1:numberOfLoopRepetitions % repeat visualization loop
    for i = 1:length(q.LF)
           
%             need to change something in IK for this formulation to work
%             T_HB.(EEselection)(:,:,i) = meanCyclicC_IBody(:,:,i)
            T_HB = eye(4);
             
            
            T_H1.(EEselection)(:,:,i) = [1, 0,          0,          0;
                0, cos(q.(EEselection)(i,1)),   -sin(q.(EEselection)(i,1)),  0;
                0, sin(q.(EEselection)(i,1)),    cos(q.(EEselection)(i,1)),  0;
                0, 0,          0,          1];
            
            % transformation from HFE to HAA
            % rotation about Hy, translation along hip link
            T_12.(EEselection)(:,:,i) = [cos(q.(EEselection)(i,2)), 0,  sin(q.(EEselection)(i,2)),  0;
                0,         1,  0,          0;
                -sin(q.(EEselection)(i,2)), 0,  cos(q.(EEselection)(i,2)), -l_hip;
                0,         0,  0,          1];
            ...
            setFixedTransform(jnt2, T_HB);
            setFixedTransform(jnt3, T_H1.(EEselection)(:,:,i));
            setFixedTransform(jnt4,  T_12.(EEselection)(:,:,i));
            setFixedTransform(jnt5, T_23.(EEselection)(:,:,i));
            setFixedTransform(jnt6, T_34.(EEselection)(:,:,i));
