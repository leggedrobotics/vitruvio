function [electricalPower, operatingPointEfficiency] = computeElectricalPowerInput(Leg, EEselection, actuatorProperties, linkCount, actuatorEfficiency, actuatorSelection, efficiencyMapPlot)
    ...
    % if the actuator limits are exceeded, the indexes computed using
    % these ratios are beyond the limits of the efficiency map.
    ...
    for i = 1:linkCount+1 % joint [HAA, HFE, KFE, (AFE), (DFE)]                                              
        for j = 1:length(torqueIndex(:,i)) % time step    
            ...
            if (torqueIndex(j,i) > length(actuatorEfficiency.(actuator{i}))) || (qdotIndex(j,i) > length(actuatorEfficiency.(actuator{i})))
                operatingPointEfficiency(j,i) = nan;
            else
