...
% compute joint/actuator/motor requirements, optimize the leg design, plot, and visualize the motion. 
    ...
    load(dataSelection);
    dt = t(2) - t(1); % time step is constant for motions generated in Towr
    Leg.time = t;
    for i = 1:legCount
    ...
    Leg.robotProperties.computeEfficiencyMaps = computeEfficiencyMaps; 
    ...
    if absoluteMaxPointDeviation > allowableDeviation
        ...
        for j = 1:numberOfTimesPointsAreDoubled
            ...
            dt = dt/2; % Halve the time step each time a point is interpolated
            Leg.time = [0:dt:t(end)]';
        end
    ...
    for i = 1:legCount
        ...
        % Base velocity from finite differences of base position.
    ...
    %% (Optional) qAFE, qDFE torque based heuristic computation
    ...
    for i = 1:legCount
        EEselection = EEnames(i,:);
        Leg.(EEselection).rigidBodyModelSwing = buildRobotRigidBodyModel(gravity, actuatorProperties, actuateJointDirectly, linkCount, robotProperties, Leg, EEselection);
    end
    ...
    %% Get joint torques for stance and swing phase then concatenate them
    fprintf('Computing joint torques. \n');
    for i = 1:legCount
        EEselection = EEnames(i,:);
        externalForce = Leg.(EEselection).force(:,1:3); 
        Leg.(EEselection).jointTorqueStance = getStanceJointTorques(externalForce, Leg, EEselection, meanCyclicMotionHipEE); % Returns zeros when foot is not in contact with the ground.
        Leg.(EEselection).jointTorqueSwing = getSwingJointTorques(EEselection, Leg, meanCyclicMotionHipEE, linkCount); % Returns zeros when foot is in contact with the ground.
    ...
    %% (Optional) Get active and passive torques for spring modeled in parallel with joint
    ...
    %% (Optional) Get actuator torque and speed as result of gearing between actuator and joint
    ...
    for i = 1:legCount
        ...
        for j = 1:jointCount
            % gear ratio = actuator speed/joint speed
...
%% (Optional) Get efficiency map of actuator and electrical power
    ...
    fprintf('Computing actuator efficiency map. \n');
        ...
        % Assume efficiency of 1.
        for i = 1:length(actuatorList)
            actuatorName = actuatorList(i);
            actuatorEfficiency.(actuatorName{1}) = 1;
            Leg.efficiencyMapPlot.(actuatorName{1}) = 1;
        end

        for i = 1:legCount
            EEselection = EEnames(i,:);
            Leg.(EEselection).elecPower = Leg.(EEselection).jointPower;
            Leg.(EEselection).operatingPointEfficiency = ones(size(Leg.(EEselection).jointPower));
        end
    end
    ...
    for i = 1:legCount
        ...
        % Initialize totalLegMassOpt to just the mass of actuators and EE which is
        % (totalLegMass - totalLinkMass)
        ...
        % The optimized mass is the nominal mass + change in leg mass
        Leg.robotProperties.mass.totalOpt = Leg.robotProperties.mass.total + (Leg.robotProperties.mass.totalLegMassOpt - Leg.robotProperties.mass.totalLegMass);
        for i = 1:legCount
            EEselection = EEnames(i,:);
            if optimizeLeg.(EEselection)
                power = Leg.(EEselection).jointPowerOpt;
